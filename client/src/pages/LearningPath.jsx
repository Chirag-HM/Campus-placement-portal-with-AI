...
export default function LearningPath() {
  ...
  const [activeTab, setActiveTab] = useState('roadmap'); // 'roadmap' or 'hub'

  useEffect(() => {
    api.get('/learning/my-path').then(r => setPath(r.data.path)).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex gap-4 mb-8">
        <button 
          ...
          className={`px-6 py-3 rounded-2xl font-bold transition-all ${activeTab === 'roadmap' ? 'bg-primary text-white shadow-glow shadow-primary/20' : 'bg-white/5 text-text-muted hover:bg-white/10'}`}
        >
          My Roadmap
        </button>
        <button 
          onClick={() => setActiveTab('hub')}
          className={`px-6 py-3 rounded-2xl font-bold transition-all ${activeTab === 'hub' ? 'bg-primary text-white shadow-glow shadow-primary/20' : 'bg-white/5 text-text-muted hover:bg-white/10'}`}
        >
          Courses & Certs
        </button>
      </div>
...
function CourseHub() {
  ...
  useEffect(() => {
    api.get('/learning/courses').then(r => setCourses(r.data.courses)).catch(() => {}).finally(() => setLoading(false));
  }, []);

  const handleComplete = async (courseId) => {
    try {
      await api.post('/learning/courses/complete', { courseId });
      // Refresh logic or toast
    } catch { /* ignore */ }
  };

  if (loading) return <div className="grid md:grid-cols-2 gap-6">{[...Array(4)].map((_, i) => <div key={i} className="skeleton h-48 rounded-2xl" />)}</div>;

  return (
    <div className="space-y-12">
      <section className="grid md:grid-cols-2 gap-6">
        {courses.map((c, i) => (
          <motion.div key={c._id} custom={i} initial="hidden" animate="visible" variants={fadeUp}
            className="glass-card p-6 flex flex-col group border-white/5 hover:border-primary/30 transition-all">
            <div className="flex justify-between items-start mb-4">
              <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                c.level === 'Beginner' ? 'bg-emerald/10 text-emerald' : 
                c.level === 'Intermediate' ? 'bg-blue/10 text-blue-400' : 'bg-purple/10 text-purple-400'
              }`}>
                {c.level}
              </span>
              <span className="text-[10px] font-black text-text-muted uppercase tracking-widest">{c.duration}</span>
            </div>
            <h3 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors">{c.title}</h3>
            <p className="text-text-secondary text-sm mb-6 line-clamp-2">{c.description}</p>
            ...
            <div className="mt-auto flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold">+{c.xpReward} XP</span>
              </div>
              <button onClick={() => handleComplete(c._id)} className="btn-primary py-2 px-6 text-sm">Start Course</button>
            </div>
          </motion.div>
        ))}
      </section>
      ...
      <section>
        <h2 className="font-display text-2xl font-bold mb-6">Your Certifications</h2>
        <div className="glass-card p-8 text-center border-dashed border-2 border-white/5">
          <FileText className="w-12 h-12 text-text-muted mx-auto mb-4" />
          <p className="text-text-secondary">Complete courses to earn official PlaceAI certifications.</p>
        </div>
      </section>
    </div>
  );
